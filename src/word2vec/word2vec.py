...
def negSamplingLossAndGradient(
    ...
    theta = outsideVectors[indices]
    sig = - np.dot(centerWordVec, theta.T)
    sig[0] = -sig[0]
    sig = sigmoid(sig)
    loss = - np.sum(np.log(sig))

    center_sig = 1 - sig
    center_sig[0] = -center_sig[0]
    gradCenterVec = np.dot(center_sig,theta)

    g_outside = np.dot(center_sig[:, np.newaxis] , centerWordVec[np.newaxis, :])
    gradOutsideVecs = np.zeros_like((outsideVectors))
    for i in range(len(indices)):
      gradOutsideVecs[indices[i]] += g_outside[i]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outword in outsideWords:
      (C_loss, gradCenter, gradOutside) = word2vecLossAndGradient(
          centerVec, word2Ind[outword], outsideVectors, dataset)
      loss += C_loss
